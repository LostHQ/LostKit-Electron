...
let tabs = [
    {
        id: "main",
        url: "https://w2-2004.lostcity.rs/rs2.cgi?plugin=0&world=2&lowmem=0",
        title: "W2 HD",
    },
];
let tabByUrl = new Map([
    ["https://w2-2004.lostcity.rs/rs2.cgi?plugin=0&world=2&lowmem=0", "main"],
]);
...
app.whenReady().then(() => {
    ...
    mainView.webContents.loadURL(
        "https://w2-2004.lostcity.rs/rs2.cgi?plugin=0&world=2&lowmem=0",
    );
        ...
        switch (view) {
            case "worldswitcher":
                navView.webContents.loadFile(
                    path.join(__dirname, "/navitems/worldswitcher.html"),
                );
                break;
            case "hiscores":
                navView.webContents.loadFile(
    ...
    ipcMain.on("open-external", (event, url, title) => {
        ...
        win.on("closed", () => {
            if (externalWindowsByUrl.get(url) === win)
